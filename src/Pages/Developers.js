        ...
        <div className="container mx-auto px-4 py-2 flex justify-between items-center">
          ...
          <nav className={`md:block ${isMenuOpen ? 'block' : 'hidden'}`}>
            <ul className="flex flex-col md:flex-row space-y-4 md:space-y-0 md:space-x-6 text-gray-600">
              {[
                { icon: faLanguage, text: "Translation", to: "/translation" },
                { icon: faBook, text: "Dictionary", to: "/dictionary" },
                { icon: faInfoCircle, text: "About Us", to: "/about-us" },
                { icon: faUsers, text: "Community", to: "/community" },
                { icon: faCode, text: "API", to: "/api" }
              ].map((item, index) => (
                <li key={index}>
                  <Link to={item.to} className="hover:text-gray-300 flex items-center">
                    <FontAwesomeIcon
                      icon={item.icon}
                      className="mr-1 text-2xl transition duration-600 transform hover:scale-125"
                    />
                    {item.text}
                  </Link>
                </li>
          ...
          <div className="flex items-center login_button space-x-4">
            <button className="bg-blue-500 text-white px-4 py-2 rounded-full font-bold hover:bg-blue-400 transition duration-300">Login</button>
